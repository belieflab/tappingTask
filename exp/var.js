...
for (let i = 0; i < 10; i++){
    countdown_practice_stim.push({stimulus: countdownTrial[i], stim: tone.slice(5,15), data: {test_part: 'practice'}},)
}

// adds sound to the countdown stimuli to be passed to the countdown object
countdown_experiment_stim = [];
for (let i = 0; i < 10; i++){
    countdown_experiment_stim.push({stimulus: countdownTrial[i], stim: tone.slice(5,15), data: {test_part: 'experiment'}},)
}